...
class hzMainWindowsDlg(QMainWindow, Ui_MainWindow):
    ...
    def folderBrowse(self):
        ...
        Return(s):
                    None
        Notes:
                    (Used Module) from PyQt5.QtWidgets import QFileDialog
                    2016-09-24 V1.0[Heyn]
        '''
        directory = QFileDialog.getExistingDirectory(
            self, "Regex Folder", QtCore.QDir.currentPath())
    ...
    def signeltableWidgetUpdate(self, msgList):
        ...
        for index in range(len(msgList)):
            ...
            self.tableWidget.setItem(
                newRowCount, 0, QTableWidgetItem(msgList[index][0]))
            self.tableWidget.setItem(
                newRowCount, 1, QTableWidgetItem(msgList[index][1]))
        pass
